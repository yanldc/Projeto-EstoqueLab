...
function ReagentSpecList({ id }) {
  const [reagentSpecs, setReagentSpecs] = useState([]);
  const [error, setError] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [editingId, setEditingId] = useState(null);
  const [newData, setNewData] = useState({ quantity: '', lote:'', price: '', validity:'', dateBuy: '', nf: '', supplier: '' });
  const itemsPerPage = 15;
  const [quantityCondition, setQuantityCondition] = useState('greaterThanZero'); 

  const fetchReagentSpecs = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:3001/reagentSpec/${id}?page=${currentPage}&limit=${itemsPerPage}&quantityCondition=${quantityCondition}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      const { reagentSpecs: fetchedReagentSpecs, totalPages: fetchedTotalPages } = response.data;
      setReagentSpecs(fetchedReagentSpecs);
  ...
  const handleDelete = async (specId) => {
    ...
    if (confirmDelete) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://localhost:3001/reagentSpec/${id}/${specId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleUpdate = async (specId) => {
    ...
    if (confirmUpdate) {
      try {
        const token = localStorage.getItem('token');
        await axios.put(
          `http://localhost:3001/reagentSpec/${id}/${specId}`,
          {
        ...
        setEditingId(null);
        setNewData({ quantity: '', lote:'', price: '', validity:'', dateBuy: '', nf: '', supplier: '' });
        fetchReagentSpecs();
      } catch (err) {
                  ...
                  <td>
                    {editingId === spec.id ? (
                      <input
                        type="text"
                        value={newData.lote}
                        onChange={(e) => setNewData({ ...newData, lote: e.target.value })}
                      />
                    ) : (
                      spec.nf
                    )}
                  ...
                  <td>
                    {editingId === spec.id ? (
                      <input
                        ...
                        onChange={(e) => setNewData({ ...newData, validity: e.target.value })}
                      />
                    ) : (
                      spec.dateBuy
                    )}
...
export default ReagentSpecList;
